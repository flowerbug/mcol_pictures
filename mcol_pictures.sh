...
for coll_dir_name in ${list_coll} ; do
  ...
  if test "${count_them}" == "0" ; then
    ...
    if test ${link_files} == "1" ; then 
      ...
      exiftool ${verb} -P ${PIC_INDEX} -fileOrder FileModifyDate -d ${PIC_INDEX}/%Y/%m/%d/${coll_dir_name}-%Y%m%d-%H%M%S%z '-HardLink<$FileModifyDate-$ShutterCount.%e' ${PIC_COLL}/${coll_dir_name}/*
    elif test ${copy_files} == "1" ; then
      echo -e "\n  Copying collection ${coll_dir_name}...\n\n"
      exiftool ${verb} -P -o ${PIC_INDEX} -fileOrder FileModifyDate -d ${PIC_INDEX}/%Y/%m/%d/${coll_dir_name}-%Y%m%d-%H%M%S%z '-FileName<$FileModifyDate-$ShutterCount.%e' ${PIC_COLL}/${coll_dir_name}/*
    elif test ${move_files} == "1" ; then
      echo -e "\n  Moving collection ${coll_dir_name}...\n\n"
      exiftool ${verb} -P ${PIC_INDEX} -fileOrder FileModifyDate -d ${PIC_INDEX}/%Y/%m/%d/${coll_dir_name}-%Y%m%d-%H%M%S%z '-FileName<$FileModifyDate-$ShutterCount.%e' ${PIC_COLL}/${coll_dir_name}/*
    else
